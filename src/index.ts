...
async function main(): Promise<void> {
  ...
  if (isDemoMode()) {
    ...
    client = new FixtureFhirClient();
    detail = "DEMO fixtures — no credentials, no PHI";
  } else {
    ...
    audit = new AuditLogger(config.auditLogPath, config.auditLogFormat);
    ...
    client = new FhirClient(config.fhirBaseUrl, tokens, {
      retryMaxAttempts: config.retryMaxAttempts,
      retryBaseMs: config.retryBaseMs,
      retryCapMs: config.retryCapMs,
    });
    detail = `audit=${config.auditLogPath ? "file" : "stderr"}, retry=${config.retryMaxAttempts}/${config.retryBaseMs}ms-${config.retryCapMs}ms`;
  }
